...
def main() -> None:
    ...
    with start_mlflow_run(
        ...
        default_run_name="latency-benchmark",
        default_tags={"stage": "evaluation", "script": "evaluation/latency_benchmark.py"},
    ) as tracker:
        ...
        tracker.log_metrics(
            {
                sanitize_metric_name(f"{stage_id}.{metric_name}"): metric_value
                for stage_id, summary in stages.items()
                for metric_name, metric_value in summary.items()
                if metric_name != "display_name" and metric_name != "stage_id"
            }
        )
        tracker.log_artifact(benchmark_config["output_path"], artifact_path="outputs")
